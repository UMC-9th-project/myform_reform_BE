...
export class OrdersService {
  ...
  private validateOptions(
    ...
    for (const optionItem of optionItems) {
      ...
      if (optionGroupIds.has(groupId)) {
        throw new ItemNotFoundError('동일 옵션 그룹에서 여러 옵션을 선택할 수 없습니다.');
      }
  ...
  private async generateReceiptNumber(): Promise<string> {
    try {
      ...
      for (let attempt = 0; attempt < maxAttempts; attempt++) {
        ...
        const existing = await this.repository.findReceiptByReceiptNumber(receiptNumber);
        if (!existing) {
  ...
  private async findReceiptByIdOrNumber(
    ...
    try {
      ...
      if (isUuid) {
        ...
        const order = await this.repository.findOrderById(orderIdOrNumber, userId);
        if (!order) {
          return null;
        }
        ...
        return await this.repository.findReceiptByReceiptNumberWithOrders(orderIdOrNumber, userId);
      }
    } catch (error) {
  ...
  async createOrder(
    ...
    try {
      ...
      const optionItemIdsWithQuantity = optionItems
        ...
        if (optionItemIdsWithQuantity.length > 0) {
          ...
          for (const optionItemId of optionItemIdsWithQuantity) {
            ...
            if (updateResult !== 1) {
              const updatedItem = await this.repository.findUpdatedOptionItems([optionItemId]);
              const optionItem = optionItems.find(item => item.option_item_id === optionItemId);
      ...
      const totalAmount = productAmount + deliveryFee;
        ...
        let receipt = await this.repository.findReceiptByReceiptNumber(receiptNumber);
        ...
        if (!receipt) {
          ...
          await this.repository.updateReceiptTotalAmount(receipt.receipt_id, totalAmount);
        }

        const initialOrderStatus = receipt.payment_status === 'paid' 
  ...
  async getOrder(orderIdOrNumber: string, userId: string): Promise<GetOrderResponseDto> {
    try {
      ...
      const receipt = await this.findReceiptByIdOrNumber(orderIdOrNumber, userId);
      ...
      if (firstOrder.user_address) {
        const address = await this.repository.findDeliveryAddressByIdDetailed(firstOrder.user_address);
        if (address) {
          deliveryAddress = {
            postal_code: address.postal_code,
            address: address.address,
      ...
      const orderItems = receipt.order.flatMap((order: {
        order_id: string;
        ...
        return order.order_option.map((orderOption: {
          option_item: {
            option_group: {
              name: string | null;
              item: {
      ...
      const maxDeliveryFee = Math.max(
        ...receipt.order.map((o: { delivery_fee: number | null }) => o.delivery_fee ? Number(o.delivery_fee) : 0)
      );
  ...
  /**
   * 포트원 API로 결제 정보 조회 (재시도 로직 포함)
   */
  private async fetchPaymentInfoWithRetry(impUid: string): Promise<PortonePaymentInfo> {
    ...
    try {
      if (receipt.order.length === 0) {
        ...
        if (paymentInfo.status === 'paid') {
          const expectedAmount = receipt.total_amount ? Number(receipt.total_amount) : 0;
          if (paymentInfo.amount === expectedAmount && paymentInfo.merchant_uid === merchantUid) {
      ...
      const allPaid = receipt.order.every(o => o.status === order_status_enum.PAID);
      if (allPaid) {
        return true;
      }

      const allPending = receipt.order.every(o => o.status === order_status_enum.PENDING);
      if (!allPending) {
        if (throwOnError) {
          throw new OrderError(
      ...
      await runInTransaction(async () => {
        ...
        await this.repository.updateOrdersStatus(orderIds, order_status_enum.PAID);
  ...
  async verifyPayment(orderId: string, impUid: string): Promise<void> {
    try {
      ...
      const receiptData = await this.repository.findReceiptByIdWithOrders(orderReceipt.receipt_id);
      if (!receiptData) {
        throw new OrderNotFoundError(orderId);
      }
      ...
      const receipt = {
        ...
        total_amount: receiptData.total_amount ? Number(receiptData.total_amount) : null,
        order: receiptData.order.map((o: {
          order_id: string;
          status: order_status_enum | null;
          quantity: number | null;
          order_option?: Array<{ option_item_id: string }>;
          ...
          status: o.status,
          quantity: o.quantity,
          order_option: o.order_option?.map((oo: { option_item_id: string }) => ({ option_item_id: oo.option_item_id }))
        }))
      };
    ...
    try {
      await runInTransaction(async () => {
        for (const order of receipt.order) {
          await this.repository.updateOrderStatus(order.order_id, order_status_enum.CANCELLED);

          if (order.order_option && order.order_option.length > 0 && order.quantity) {
            const restoreQuantity = order.quantity;
            for (const opt of order.order_option) {
              await this.repository.restoreOptionItemQuantities([opt.option_item_id], restoreQuantity);
            }
          }
        }
      ...
      console.error(`주문 취소 실패 (receiptId: ${receipt.receipt_id}, impUid: ${impUid}):`, error);
    }
  }
  ...
  async handleWebhook(impUid: string, merchantUid: string): Promise<void> {
    try {
      let receiptData = await this.repository.findReceiptByReceiptNumberForVerification(merchantUid);
      
      if (!receiptData) {
        try {
          const paymentInfo = await this.fetchPaymentInfoWithRetry(impUid);
          
          if (paymentInfo.status === 'paid' && paymentInfo.merchant_uid === merchantUid) {
            try {
              await this.repository.createReceipt({
                ...
                payment_method: 'card',
                payment_gateway: 'portone',
                transaction: paymentInfo.card_name && paymentInfo.card_number
                  ? JSON.stringify({
                      imp_uid: impUid,
                      card_name: paymentInfo.card_name,
                      card_number: paymentInfo.card_number,
              ...
              if (createError?.code === 'P2002' || createError?.message?.includes('Unique constraint')) {
                receiptData = await this.repository.findReceiptByReceiptNumberForVerification(merchantUid);
                if (receiptData) {
                  ...
                  console.error(`웹훅: receipt 생성 실패 후 조회도 실패 (merchantUid: ${merchantUid}, impUid: ${impUid})`);
                  return;
                }
              } else {
                throw createError;
            ...
            if (!receiptData) {
              console.error(`웹훅: receipt 생성/업데이트 후 조회 실패 (merchantUid: ${merchantUid}, impUid: ${impUid})`);
              return;
            }
          } else {
            // 결제가 완료되지 않았거나 merchant_uid가 일치하지 않음
            console.error(`웹훅: 결제 정보 불일치 (merchantUid: ${merchantUid}, impUid: ${impUid}, status: ${paymentInfo.status})`);
            return;
          }
        } catch (error) {
          // 포트원 API 조회 실패 시 로그만 남기고 성공 응답 (재전송 방지)
          console.error(`웹훅: receipt를 찾을 수 없고 결제 정보 조회 실패 (merchantUid: ${merchantUid}, impUid: ${impUid}):`, error);
          return;
        }
      }
      ...
      console.error(`웹훅 처리 실패 (impUid: ${impUid}, merchantUid: ${merchantUid}):`, error);
    }
  }
  ...
  async getOrderSheetFromCart(
    ...
    try {
      ...
      // 각 cart의 item 정보 조회
      const itemIds = Array.from(new Set(carts.map(c => c.item_id).filter((id): id is string => id !== null)));
      ...
      for (const cart of carts) {
        ...
        orderItems.push({
          reformer_nickname: item.owner?.nickname || '',
          thumbnail: item.item_photo.find(p => p.photo_order === 1)?.content || '',
          title: item.title || '',
          selected_options: selectedOptions,
    ...
    try {
      ...
      const result = await runInTransaction(async () => {
        ...
        for (const cart of carts) {
          ...
          const allOptionItems = await this.repository.findOptionItemsByIds(optionItemIds);
        ...
        const ownerIds = Array.from(new Set(orderDataList.map(od => od.item.owner_id)));
        ...
        for (const orderData of orderDataList) {
          orderData.deliveryAddressId = deliveryAddressMap.get(orderData.item.owner_id);
        }

        const totalAmount = totalProductAmount + maxDeliveryFee;
        let receipt = await this.repository.findReceiptByReceiptNumber(merchantUid);
        ...
        for (const orderData of orderDataList) {
          const { cart, item, optionItemIds, productAmount, deliveryFee, deliveryAddressId } = orderData;
          ...
          if (optionItemIdsWithQuantity.length > 0) {
            for (const optionItemId of optionItemIdsWithQuantity) {
              ...
              if (updateResult !== 1) {
                const updatedItem = await this.repository.findUpdatedOptionItems([optionItemId]);
                const optionItem = allOptionItems.find(item => item.option_item_id === optionItemId);
          ...
          createdOrders.push({ order_id: order.order_id, item_id: cart.item_id! });
        }
